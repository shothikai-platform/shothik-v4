...
export async function POST(request: Request) {
  try {
    ...
    if (chatId) {
      try {
        // SECURITY: Fix IDOR by ensuring the requested session belongs to the authenticated user
    ...
    const conversation = await SheetConversation.create({
      ...
      prompt,
      events: [
        { step: "init", message: "Request received", timestamp: new Date() },
      ],
      status: "generating",
    });
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          sendJSON({
            data: {
              message: "Generating spreadsheet structure...",
              step: "llm_processing",
            },
          });

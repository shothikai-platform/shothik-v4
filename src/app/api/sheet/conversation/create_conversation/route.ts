    ...
    try {
        ...
        if (chatId) {
            try {
                ...
                if (!session) {
                    // If session ID provided but not found/owned, we could error or fall back to creating new.
                    // Returning 404 is safer to indicate "Access Denied" effectively.
        ...
        if (!session) {
            session = await SheetSession.create({
                userId: user._id || user.id, // Security: Use actual user ID

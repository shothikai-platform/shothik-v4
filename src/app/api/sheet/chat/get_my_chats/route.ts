...
export async function GET(request: Request) {
    try {
        ...
        // Impact: Reduces memory overhead and execution time when fetching multiple records
        const sessions = await SheetSession.find({}).sort({ updatedAt: -1 }).lean();

        // Map over the lean results to re-add the `id` field expected by the frontend

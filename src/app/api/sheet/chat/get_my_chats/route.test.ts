...
describe('GET /api/sheet/chat/get_my_chats', () => {
  beforeEach(() => {
    ...
    mockLean.mockResolvedValue([{ _id: '1', title: 'Chat 1', status: 'active', createdAt: new Date(), updatedAt: new Date() }]);
  ...
  it('should fetch chats scoped to user and optimized', async () => {
    ...
    // Verify select is called with correct fields
    ...
    // Verify response format (title -> name mapping)
    expect(response.data[0]).toHaveProperty('name', 'Chat 1');
    expect(response.data[0]).toHaveProperty('title', 'Chat 1');
  });
});

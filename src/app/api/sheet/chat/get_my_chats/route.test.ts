...
describe('GET /api/sheet/chat/get_my_chats', () => {
  ...
  it('should return 401 if user is not authenticated', async () => {
    ...
    // Currently it returns 200 (error) or 500, but certainly not 401
    expect(response.status).toBe(401);
  });

  it('should fetch sessions filtered by userId', async () => {
    const mockUser = { _id: 'user123' };
    (getAuthenticatedUser as any).mockResolvedValue(mockUser);

    await GET(new Request('http://localhost/api/sheet/chat/get_my_chats'));

    // This expectation ensures we are filtering by userId, which is critical for performance and security

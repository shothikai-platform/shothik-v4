...
describe('GET /api/sheet/chat/get_my_chats', () => {
    beforeEach(() => {
        ...
        mockLean.mockResolvedValue([{ id: 'session1', title: 'Test Session' }]);
    ...
    it('should return user sessions and apply lean for performance', async () => {
        ...
        expect(response.data).toEqual([{ id: 'session1', title: 'Test Session' }]);
    ...
    it('should handle user id coming from user.id fallback', async () => {
        const mockUser = { id: 'user123_fallback', name: 'Test User' };
        (getAuthenticatedUser as any).mockResolvedValue(mockUser);

        await GET(new Request('http://localhost/api/sheet/chat/get_my_chats'));

        expect(mockFind).toHaveBeenCalledWith({ userId: 'user123_fallback' });
    });

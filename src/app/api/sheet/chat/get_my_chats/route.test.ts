...
describe('GET /api/sheet/chat/get_my_chats', () => {
  beforeEach(() => {
    ...
    mockSort.mockResolvedValue([{ _id: '1', title: 'Sheet Chat 1' }]);
  ...
  it('should fetch chats filtered by userId for performance and security', async () => {
    const mockUser = { _id: 'user123' };
    (getAuthenticatedUser as any).mockResolvedValue(mockUser);

    await GET(new Request('http://localhost/api/sheet/chat/get_my_chats'));

    // This is the core verification: find MUST be called with userId

...
describe('GET /api/sheet/chat/get_my_chats', () => {
  ...
  it('should fetch chats scoped to the user and use lean()', async () => {
    const mockUser = { _id: 'user123' };
    (getAuthenticatedUser as any).mockResolvedValue(mockUser);

    await GET(new Request('http://localhost/api/sheet/chat/get_my_chats'));

    // Assert that find was called with the correct userId scope
    expect(mockFind).toHaveBeenCalledWith({ userId: 'user123' });

    // Assert that sort was called
    expect(mockSort).toHaveBeenCalledWith({ updatedAt: -1 });

    // Assert that lean was called for performance

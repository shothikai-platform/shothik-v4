...
describe('PUT /api/research/chat/update_name/[id]', () => {
  ...
  it('should return 400 if name is missing or empty', async () => {
    ...
    expect((response as any).data.error).toBe('Name is required');
  });
  ...
  it('should return 400 if name exceeds 100 characters', async () => {
    ...
    expect(response.status).toBe(400);
    expect((response as any).data.error).toBe('Name is too long');
  });

  it('should return 404 if chat is not found or does not belong to the user (IDOR prevention)', async () => {
    (getAuthenticatedUser as any).mockResolvedValue({ _id: 'user1' });
    ...
    expect((response as any).data.error).toBe('Chat not found');
  });

  it('should return 200 and the updated chat upon success', async () => {
    const mockUser = { _id: 'user1' };
    const mockChat = { _id: 'chat1', userId: 'user1', name: 'Updated Name' };

...
describe('PUT /api/research/chat/update_name/[id]', () => {
  ...
  it('should return 401 if user is not authenticated', async () => {
    ...
    // Even if chat exists, it should return 401
    mockFindByIdAndUpdate.mockResolvedValue({ _id: 'chat1', userId: 'user1', name: 'Some Name' });
  ...
  it('should return 404 if chat belongs to another user or does not exist', async () => {
    ...
    // findOneAndUpdate will return null if the query { _id: id, userId: user2 } fails
    mockFindOneAndUpdate.mockResolvedValue(null);
    // findByIdAndUpdate might still return something (IDOR), but we want to test the FIXED state
    mockFindByIdAndUpdate.mockResolvedValue({ _id: 'chat1', userId: 'user1', name: 'Other User Chat' });
  ...
  it('should successfully update chat name if authenticated and owner', async () => {

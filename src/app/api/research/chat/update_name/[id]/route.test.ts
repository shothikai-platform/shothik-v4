...
describe('PUT /api/research/chat/update_name/[id]', () => {
  ...
  it('should return 400 if name is too long (greater than 100 characters)', async () => {
    (getAuthenticatedUser as any).mockResolvedValue({ _id: 'user123' });
    ...
    expect(response.status).toBe(400);
    expect((response as any).data).toEqual({ error: 'Name too long' });
  });
  ...
  it('should return 404 if chat is not found or user does not own it (IDOR protection)', async () => {
    ...
    const req = new Request('http://localhost/api/research/chat/update_name/chat1', {
      method: 'PUT',
      body: JSON.stringify({ name: 'Valid New Title' }),
    });
  ...
  it('should update and return chat document on valid request', async () => {
    const mockUser = { _id: 'user123' };
    (getAuthenticatedUser as any).mockResolvedValue(mockUser);

    const mockUpdatedChat = { _id: 'chat1', userId: 'user123', name: 'Valid New Title' };

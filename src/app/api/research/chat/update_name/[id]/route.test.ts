...
describe('PUT /api/research/chat/update_name/[id]', () => {
  ...
  it('should return 401 if user is not authenticated', async () => {
    ...
    // Currently returns 200, we want it to be 401
    expect(response.status).toBe(401);
  });

  it('should return 404 if chat belongs to another user', async () => {
    (getAuthenticatedUser as any).mockResolvedValue({ _id: 'user2' });
    // In current insecure code, findByIdAndUpdate is used, which doesn't check owner.
    // So it would return the chat and status 200.
    mockFindByIdAndUpdate.mockResolvedValue({ _id: 'chat1', userId: 'user1', name: 'Old Name' });
    ...
    // Currently returns 200 (via findByIdAndUpdate), we want it to be 404

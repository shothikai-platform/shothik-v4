...
describe('PUT /api/research/chat/update_name/[id]', () => {
  ...
  it('should return 404 if chat belongs to another user', async () => {
    ...
    // In secure version, findOneAndUpdate should be called with userId
  ...
  it('should correctly update name (not title) and verify ownership', async () => {
      const mockUser = { _id: 'user1' };
      (getAuthenticatedUser as any).mockResolvedValue(mockUser);
      mockFindOneAndUpdate.mockResolvedValue({ _id: 'chat1', name: 'Updated Name', userId: 'user1' });

...
describe('DELETE /api/research/chat/delete_chat/[id]', () => {
  ...
  it('should return 404 if chat belongs to another user', async () => {
    ...
    // simulate that findOneAndDelete returns null when filtered by user2
    mockFindOneAndDelete.mockResolvedValue(null);
    // however, the insecure version uses findByIdAndDelete which would find it if it exists
    ...
    // Desired behavior: 404 (Not Found or Unauthorized)
    expect(response.status).toBe(404);
  });

  it('should return 200 if user owns the chat', async () => {

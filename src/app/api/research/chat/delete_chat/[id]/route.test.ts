...
describe('DELETE /api/research/chat/delete_chat/[id]', () => {
  ...
  it('should return 401 if user is not authenticated', async () => {
    ...
    // Even if it returns a chat, it should fail before that due to auth check
  ...
  it('should return 404 if chat belongs to another user or does not exist', async () => {
    ...
    // Mock that findOneAndDelete returns null when searching by ID and user2
    mockFindOneAndDelete.mockResolvedValue(null);

    // In insecure mode, it uses findByIdAndDelete
    mockFindByIdAndDelete.mockResolvedValue({ _id: 'chat1' });
    ...
    // Current behavior: 200 (deletes chat found by ID)
    // Desired behavior: 404 (Not Found via findOneAndDelete with ownership check)

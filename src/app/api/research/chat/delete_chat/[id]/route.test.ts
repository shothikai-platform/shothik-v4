...
describe('DELETE /api/research/chat/delete_chat/[id]', () => {
  ...
  it('should return 401 if user is not authenticated', async () => {
    ...
    // Current behavior: 200 (if it doesn't check auth)
  ...
  it('should return 404 if chat belongs to another user or does not exist', async () => {
    ...
    // Simulate that the chat doesn't exist for user2
    mockFindOneAndDelete.mockResolvedValue(null);
    // If it still uses findByIdAndDelete, we might see it return the chat of user1
    ...
    // Current behavior: 200 (if it uses findByIdAndDelete and finds it)
    // Desired behavior: 404 (Not Found via findOneAndDelete scoped to user)

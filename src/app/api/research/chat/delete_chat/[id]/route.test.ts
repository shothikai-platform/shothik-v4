...
describe('DELETE /api/research/chat/delete_chat/[id]', () => {
  ...
  it('should delete and return success if chat belongs to user', async () => {
    (getAuthenticatedUser as any).mockResolvedValue({ _id: 'user1' });
  ...
  it('should return 500 with generic error message on server failure', async () => {
    (getAuthenticatedUser as any).mockResolvedValue({ _id: 'user1' });
    mockFindOneAndDelete.mockRejectedValue(new Error('Database corruption or connection lost'));
    ...
    expect(response.data).toEqual({ error: 'Failed to delete chat' });
  });
});

...
describe('DELETE /api/research/chat/delete_chat/[id]', () => {
  ...
  it('should return 404 (or 403) if chat belongs to another user', async () => {
    (getAuthenticatedUser as any).mockResolvedValue({ _id: 'user2' });

    // The insecure implementation uses findByIdAndDelete, which would return the chat (success)
    mockFindByIdAndDelete.mockResolvedValue({ _id: 'chat1', userId: 'user1' });

    // The secure implementation uses findOneAndDelete, which should return null if criteria mismatch
    ...
    // If insecure: returns 200 (deleted)
    // If secure: returns 404 (not found)

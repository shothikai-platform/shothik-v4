...
describe('DELETE /api/research/chat/delete_chat/[id]', () => {
  ...
  it('should return 401 if user is not authenticated', async () => {
    ...
    // Current behavior: 200 (deletes chat)
  ...
  it('should return 404 if chat belongs to another user', async () => {
    ...
    // The db returns a chat belonging to user1 IF findByIdAndDelete is used
    mockFindByIdAndDelete.mockResolvedValue({ _id: 'chat1', userId: 'user1', name: 'Chat 1' });

    // If findOneAndDelete is used with correct query, it should return null (simulating not found for this user)
    ...
    // Current behavior: 200 (deletes chat found by findByIdAndDelete)
    // Expected secure behavior: 404 (Not Found via findOneAndDelete)
  ...
  it('should return 200 if chat belongs to the user', async () => {
    ...
    // Note: Since I haven't implemented the fix yet, this expectation might fail if I add it now.
    // But the test suite as a whole is expected to fail.
  });
});

...
describe('DELETE /api/research/chat/delete_chat/[id]', () => {
  ...
  it('should return 404 if chat belongs to another user', async () => {
    ...
    mockFindOneAndDelete.mockResolvedValue(null); // Simulation of secure query failing
  ...
  it('should return 200 if user owns the chat', async () => {
    ...
    // In secure version, we use findOneAndDelete
    mockFindOneAndDelete.mockResolvedValue({ _id: 'chat1', userId: 'user1' });
    // In insecure version, we use findByIdAndDelete
    mockFindByIdAndDelete.mockResolvedValue({ _id: 'chat1', userId: 'user1' });

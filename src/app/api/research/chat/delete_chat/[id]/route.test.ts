...
describe('DELETE /api/research/chat/delete_chat/[id]', () => {
  ...
  it('should return 401 if user is not authenticated', async () => {
    ...
    mockFindByIdAndDelete.mockResolvedValue({ _id: 'chat1' }); // Simulate successful delete if not checked
  ...
  it('should return 404 if chat belongs to another user', async () => {
    ...
    // Insecure implementation calls findByIdAndDelete, so we mock it to return something (deletion success)
    mockFindByIdAndDelete.mockResolvedValue({ _id: 'chat1', userId: 'user1' });

    // Secure implementation calls findOneAndDelete, so we mock it to return null (not found for this user)

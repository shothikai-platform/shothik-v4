...
describe('DELETE /api/research/chat/delete_chat/[id]', () => {
  ...
  it('should successfully delete chat if user is authenticated and is the owner', async () => {
    (getAuthenticatedUser as any).mockResolvedValue({ _id: 'user1' });
    mockFindOneAndDelete.mockResolvedValue({ _id: 'chat1', userId: 'user1', name: 'My Chat' });
  ...
  it('should return 500 with generic error message on database error', async () => {
    (getAuthenticatedUser as any).mockResolvedValue({ _id: 'user1' });
    mockFindOneAndDelete.mockRejectedValue(new Error('Database breakdown'));

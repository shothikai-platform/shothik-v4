...
export async function POST(request: Request) {
  try {
    ...
    const chat = await ResearchChat.findOne({ _id: chatId, userId: user._id });
    if (!chat) {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          sendStep({
            step: "init",
            data: { jobId, message: "Research started" },
          });
          ...
          sendStep({
            step: "planning",
            data: { message: "Generating search queries..." },
          });
          ...
          sendStep({
            step: "searching",
            data: { message: "Searching the web...", queries: [query] },
          });

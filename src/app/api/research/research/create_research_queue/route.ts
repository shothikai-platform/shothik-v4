...
export async function POST(request: Request) {
    try {
        ...
        if (!chatId || !query) {
            return NextResponse.json({ error: 'Missing chatId or query' }, { status: 400 });
        }

        await dbConnect();

        // 1. Validate chat exists and belongs to user (IDOR protection)
        ...
        const stream = new ReadableStream({
            async start(controller) {
                ...
                try {
                    ...
                    // Update DB with Assistant Message (scoped by userId for safety)

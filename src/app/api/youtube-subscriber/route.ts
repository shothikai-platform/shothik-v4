...
export async function GET() {
  try {
    ...
    if (!response.ok) {
      throw new Error("Failed to fetch subscriber count from YouTube API");
    }
    ...
    return NextResponse.json({ error: "An error occurred fetching subscriber count" }, { status: 500 });
  }
}

...
const MessageBubble = memo(({ message, isLastData, isDataGenerating }) => {
  const htmlContent = useMemo(() => {
    // Ensure message is a string before passing to marked
  ...
  return (
    <div className="flex w-full items-start">
      ...
      >
        ...
        {/* Legacy support: checks if message has sources if it's an object */}
...
const ResearchContent = ({ currentResearch, isLastData, onSwitchTab }) => {
  const researchResult =
    currentResearch?.result || currentResearch?.answer || "";

  // Optimize selectors to avoid re-rendering on unrelated state changes
  ...
  const currentChatId = useSelector((state) => state.researchChat.currentChatId);
  ...
  const isDataGenerating = isStreaming || isPolling;

...
const MessageBubble = ({ message, isLastData, isDataGenerating }) => {
  ...
  useEffect(() => {
    ...
    // Use async sanitization if needed, but here it's sync.
    // Ensure we handle the promise if marked returns one (rare but possible in some setups, but usually string)
    // marked 16 returns string or Promise if async option is on. Let's assume sync.
    if (typeof rawHtml === "string") {
      setSanitizedMessage(sanitizeHtml(rawHtml));
    } else {
      Promise.resolve(rawHtml).then((html) =>
        setSanitizedMessage(sanitizeHtml(html)),
      );
    }
  }, [message]);

...
const GrammarCheckerContentSection = () => {
    ...
    onUpdate: ({ editor }) => {
      ...
      if (skipCheckRef.current) {
        ...
        if (newText && newText.trim() && !hasClearedOnMountRef.current) {
          ...
          // console.log("[Editor Update] User typing detected, resetting skipCheckRef");
  ...
  useEffect(() => {
    ...
    if (stateText !== editorText && stateText.trim()) {
      ...
      if (editorText === lastDispatchedTextRef.current) {
        ...
        // console.log("[Text Sync] Syncing Redux text to editor:", stateText.substring(0, 50));
      ...
      // console.log("[Text Sync] Clearing editor and issues - Redux text is empty");
      isClearingEditorRef.current = true;
  ...
  useEffect(() => {
    ...
    if (!editor) {
      ...
      const timeoutId = setTimeout(() => {
        if (!hasClearedOnMountRef.current && editor) {
          ...
          // console.log("[Initial Mount] Clearing all state on mount/reload (delayed)");
    ...
    if (currentSectionId) {
      // console.log("[Initial Mount] Removing sectionId from URL to prevent auto-load");
      setTimeout(() => {
        try {
  ...
  useEffect(() => {
    ...
    if (previousTextRef.current === stateText) {
      ...
      if (!stateText.trim() && !editorText.trim() && issues?.length > 0) {
        // console.log("[Text Empty] Safety check: Clearing issues - text is empty but issues exist");
        dispatch(setIssues([]));
        dispatch(setSelectedIssue({}));
    ...
    if (!editorText.trim() || !stateText.trim()) {
      ...
      const currentIssuesCount = issues?.length || 0;
      if (currentIssuesCount > 0) {
        // console.log("[Text Empty] Clearing issues - editor or state text is empty");
        dispatch(setIssues([]));
        dispatch(setSelectedIssue({}));
        previousIssuesCountRef.current = 0;
      }
    ...
    if (editorText.trim() !== stateText.trim() && !editorText.trim()) {
      ...
      if (currentIssuesCount > 0 && previousIssuesCountRef.current !== 0) {
        // console.log("[Mismatch] Editor empty but state has text, clearing issues");
        dispatch(setIssues([]));
        dispatch(setSelectedIssue({}));
        previousIssuesCountRef.current = 0;
      }

...
export const processContentWithReferences = (text, sources = []) => {
...
const ResearchContentWithReferences = ({
  ...
  const processedContent = useMemo(() => {
    return processContentWithReferences(contentStr, sources);
  }, [contentStr, sources]);

  const htmlContent = useMemo(() => {
    // Configure marked options
    // Note: marked is a singleton, so this affects global state.
    // Ideally we should use marked.parse(text, options) but marked API varies.
    // In marked v4+, marked(text, options) is supported.
    // But let's stick to setOptions inside here but efficiently.
    marked.setOptions({
      breaks: true,
      gfm: true,
    });

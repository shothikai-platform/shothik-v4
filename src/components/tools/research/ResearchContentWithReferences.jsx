...
// Helper function to process content and make references clickable
const processContentWithReferences = (text, sources) => {
  ...
  return text.replace(referenceRegex, (match, numbers) => {
    ...
    return refNumbers
      .map((refNum) => {
        ...
        if (sourceExists) {
          // Note: using data attributes that will be allowed by DOMPurify if added later
...
const ResearchContentWithReferences = ({
  ...
  // Memoize the processed markdown (used for both display and actions)
  const processedMarkdown = useMemo(() => {
    return processContentWithReferences(content, sources);
  }, [content, sources]);

  ...
  useEffect(() => {
    const fetchLocation = async () => {
      ...
      try {
        ...
        // 1. Try Google API via secure server endpoint
        try {
          const res = await fetch("/api/geolocation", { method: "POST" });
          if (res.ok) {
          ...
          console.warn("Google Geolocation API failed, trying fallback...", googleErr);
        }
